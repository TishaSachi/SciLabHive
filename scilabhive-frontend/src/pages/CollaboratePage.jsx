  ...
  return (
    ...
    >
      <div className="collab-modal">
        ...
        <div className="collab-modal-sub">
        ...
        <div className="invite-field">
          ...
          >
            <option value="viewer">
              Viewer — can view experiments and results
            </option>
            <option value="contributor">Contributor — can add results</option>
            <option value="editor">
              Editor — can create and edit experiments
            </option>
          </select>
        </div>
              ...
              <div key={inv.id} className="collab-card">
                ...
                <div className="collab-info">
                  <div className="collab-name">
                    {inv.collaborator_name || "Unknown"}
                  </div>
                  <div className="collab-meta">
                    invited you to <strong>{inv.experiment_title}</strong> as{" "}
                    {inv.role}
                  </div>
                </div>
...
export default function CollaboratePage() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [collabs, invs, exps, me] = await Promise.all([
          getMyCollaborators(),
          getMyInvitations(),
          getExperiments(),
          getMe(),
        ...
        setInvitations(invs);
        setCurrentUser(me);
        // Only show experiments own in the invite dropdown
        setExperiments(exps.filter((e) => e.user_id === me.id));
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const handleQuickInvite = async () => {
    ...
    try {
      ...
      const detail = err?.response?.data?.detail;
      showToast(
        detail === "Already invited" ? "Already invited!" : "Failed to send",
      );
    } finally {
  ...
  const sharedExpData = sharedExpIds.map((id) => ({
    id,
    title:
      collaborators.find((c) => c.experiment_id === id)?.experiment_title ||
      `Experiment #${id}`,
  ...
  if (loading) {
    return (
      <div>
        ...
        <div className="collab-page-sub">
          Manage your research collaborators
        </div>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[1, 2, 3].map((i) => (
            <div
    ...
    <div>
      ...
      {invitations.length > 0 && (
        <div className="invitations-banner">
          ...
          <button
            className="btn-view-invitations"
        ...
        <div>
          {collaborators.length === 0 ? (
            <div className="collab-empty">
              ...
              <div className="collab-empty-sub">
                Invite colleagues to work on experiments together
              </div>
            </div>
          ) : (
            <>
                          ...
                          <div
                            ...
                            style={{
                              background: getAvatarColor(
                                c.collaborator_name || c.invite_email,
                              ),
                            }}
                          >
                ...
                {sharedExpData.map((exp, i) => (
                  <div key={exp.id} className="shared-item">
                    ...
                    <span className="shared-count">
                      {exp.count} member{exp.count !== 1 ? "s" : ""}
                    </span>
                  </div>
                ))}
              </div>
